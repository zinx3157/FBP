...
LATEST="$(find "$HOME/Downloads" -maxdepth 1 -type d -name 'LabelOnZeWay-Before-V2.0.1-*' -print | sort | tail -1)"
if [ -z "$LATEST" ] || [ ! -d "$LATEST/labelonzeway" ]; then
  echo "No V2.0.1 rollback backup was found in Downloads."; exit 2
fi
