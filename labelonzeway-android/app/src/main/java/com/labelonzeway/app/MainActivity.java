...
public class MainActivity extends AppCompatActivity {
    private static final String APP_URL =
            "https://zinx3157.github.io/FBP/labelonzeway-v7/?v=release-2.5.4-20260913&app=android-2.5.4-clean";
    private static final String CLEAN_MARKER = "android_webview_clean_254";
    ...
    @Override protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        boolean mustClean = !getPreferences(MODE_PRIVATE).getBoolean(CLEAN_MARKER, false);
        if (mustClean) {
            ...
            try { deleteDatabase("webview.db"); deleteDatabase("webviewCache.db"); } catch (Exception ignored) {}
        }
        ...
        settings.setUserAgentString(settings.getUserAgentString() + " LabelOnZeWayAndroid/2.5.4-clean");

        if (mustClean) {
            webView.clearCache(true);
            webView.clearHistory();
            webView.clearFormData();
            getPreferences(MODE_PRIVATE).edit().putBoolean(CLEAN_MARKER, true).apply();
        }
    ...
    private boolean openExternal(Uri uri) {
        ...
        catch (ActivityNotFoundException error) { Toast.makeText(this, "No compatible app is installed.", Toast.LENGTH_SHORT).show(); }
        return true;
    }
    ...
    private void launchFileChooser(WebChromeClient.FileChooserParams params) {
        ...
        if (cleanTypes.size() == 1) gallery.setType(cleanTypes.get(0));
            ...
            gallery.setType("*/*");
            if (!cleanTypes.isEmpty()) gallery.putExtra(Intent.EXTRA_MIME_TYPES, cleanTypes.toArray(new String[0]));
        }
        ...
        if (acceptsImages && camera.resolveActivity(getPackageManager()) != null) {
            ...
            } catch (IOException ignored) { pendingCameraUri = null; }
        }
