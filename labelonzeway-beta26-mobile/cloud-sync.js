...
(function () {
  ...
  function loadWorkspaces() {
    ...
    return client.from('workspace_members').select('workspace_id,role,workspaces(name)').eq('user_id', session.user.id).then(function (result) {
      ...
      var activeWorkspace = workspaces.find(function (item) { return item.id === workspaceId; }) || workspaces[0];
      workspaceName = activeWorkspace.name; currentRole = activeWorkspace.role || '';
  ...
  function injectUI() {
    ...
    document.getElementById('cloud-workspace').addEventListener('change', function (event) {
      workspaceId = event.target.value; var activeWorkspace = workspaces.find(function (item) { return item.id === workspaceId; }) || {};workspaceName = activeWorkspace.name || '';currentRole = activeWorkspace.role || '';
  ...
  api.getStatus = function () { return { configured: configured(), signedIn: !!session, workspaceId: workspaceId, workspaceName: workspaceName, role: currentRole, deviceId: deviceId, pending: pendingForWorkspace().length, syncing: syncing }; };
  api.getAccessToken = function () { return session && session.access_token ? session.access_token : ''; };
