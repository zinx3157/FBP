...
Cross-platform worker for Windows/macOS/Linux/Raspberry Pi. It consumes authenticated
Supabase cloud_print_jobs and forwards ESC/POS bytes to the configured LAN printer.
No client device running LabelOnZeWay needs a local bridge or printer driver.
"""
...
def send_to_printer(host: str, port: int, payload: bytes, allowed_host: str, allowed_port: int,
                    connect_timeout=5, send_timeout=30):
...
class CloudPrintWorker:
    def __init__(self, config: dict):
        ...
        required = [self.url, self.anon_key, self.workspace_id, self.email]
        if not all(required):
    ...
    def process_one(self):
        ...
        job_id = str(job["id"])
        try:
            encoded = str(job.get("payload_base64", ""))
...
def main(argv=None):
    p = argparse.ArgumentParser(description="LabelOnZeWay cross-platform cloud print worker")
    p.add_argument("--config", default="cloud-print-worker.json")
    p.add_argument("--once", action="store_true", help="claim/process at most one queued job")
    p.add_argument("--version", action="version", version=VERSION)
    args = p.parse_args(argv)
    worker = CloudPrintWorker(load_config(args.config))
