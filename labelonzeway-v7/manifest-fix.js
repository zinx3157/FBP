(()=>{
  ...
  function refreshRowsNow(){
    ...
    $$('tbody tr',card).forEach(tr=>{
      ...
      const wanted=[phone,address].filter(Boolean).join(' · ');
      let meta=$('.v7-customer-summary',nameCell);
      if(!meta){
        meta=document.createElement('span');
        meta.className='v7-customer-summary';
        meta.textContent=wanted;
        nameCell.appendChild(meta);
      }else if(meta.textContent!==wanted){
        meta.textContent=wanted;
      }
    });
  }
  ...
  function init(){
    ...
    const nav=$('.v7-nav');
    if(nav)new MutationObserver(syncMode).observe(nav,{subtree:true,attributes:true,attributeFilter:['class']});
    const card=$('#card-manifest');
    if(card)new MutationObserver(muts=>{
      // Ignore mutations created by our own customer-summary decoration.
      const meaningful=muts.some(m=>![m.target,...m.addedNodes].some(n=>n?.nodeType===1&&n.classList?.contains('v7-customer-summary')));
      if(meaningful)refreshRows();
    }).observe(card,{subtree:true,childList:true,attributes:true,attributeFilter:['value']});
