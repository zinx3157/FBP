...
let lastHealth=null,lastDiagnostics=null,lastCheckedAt='';
...
function updateShell(data){const sync=document.querySelector('#v7-shell .v7-sync,.v7-sync');if(!sync)return;sync.dataset.renderRelay=relayReady(data)?'ready':'degraded';const current=String(sync.getAttribute('title')||'');const suffix=relayReady(data)?'Render print relay online':'Render print relay unavailable';if(!current.includes('Render print relay'))sync.setAttribute('title',(current?current+' · ':'')+suffix)}
...
function bindCloud(){const cloud=window.LabelOnZeWayCloud;if(!cloud||cloud.__renderRelayBound)return false;cloud.__renderRelayBound=true;cloud.renderRelay={baseUrl:BASE,health,diagnostics,refresh,getStatus:snapshot,isReady:()=>relayReady(lastHealth)};
  ...
  return true}
function install(){bindCloud();refresh();setInterval(refresh,60000)}
