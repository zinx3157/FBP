...
let gate=null,host=null,loginOriginalParent=null,loginOriginalNext=null,pillObserver=null,bodyObserver=null,statusTimer=null;
...
function isSignedIn(){
  ...
  return !t.includes('SIGN IN')&&(pill.classList.contains('ok')||t.includes('SYNC')||t.includes('PENDING'));
}
...
function sync(){isSignedIn()?hideGate():showGate();}
...
function install(){
  ensureGate();
  let tries=0;
  const timer=setInterval(()=>{tries++;mountLogin();if(watchPill()||tries>100)clearInterval(timer);sync();},100);
  bodyObserver=new MutationObserver(()=>{if($('#cloud-sync-pill'))watchPill();mountLogin();});
  bodyObserver.observe(document.body,{childList:true,subtree:true});
  statusTimer=setInterval(sync,400);
  document.addEventListener('visibilitychange',()=>{if(!document.hidden)sync()});
  window.addEventListener('pageshow',sync);
  document.addEventListener('click',e=>{if(e.target.closest?.('#cloud-sign-in'))setTimeout(sync,150);},true);
  sync();
}
