...
-- BEGIN authoritative migration body
...
-- END authoritative migration body

insert into supabase_migrations.schema_migrations(version, statements, name)
values (
  '20260823120000',
  array['Applied through Supabase SQL Editor from commit e3b6ae0']::text[],
  'beta3_tracking_tokens'
)
...
where schemaname = 'public'
  and tablename in ('parcel_tracking_tokens', 'parcel_tracking_projection')
order by tablename;

select n.nspname as schema_name, p.proname as function_name, pg_catalog.pg_get_function_identity_arguments(p.oid) as arguments
from pg_catalog.pg_proc p
join pg_catalog.pg_namespace n on n.oid = p.pronamespace
where n.nspname = 'public'
  and p.proname in ('upsert_parcel_tracking_projection', 'generate_parcel_tracking_token', 'revoke_parcel_tracking_token', 'public_parcel_tracking')
order by p.proname;
...
where n.nspname = 'public'
  and c.relname in ('parcel_tracking_tokens', 'parcel_tracking_projection')
order by c.relname;

select routine_name, specific_name, grantee, privilege_type
from information_schema.role_routine_grants
where routine_schema = 'public'
  and routine_name in ('upsert_parcel_tracking_projection', 'generate_parcel_tracking_token', 'revoke_parcel_tracking_token', 'public_parcel_tracking')
